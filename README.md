...
The context footer (`printContext()`, printed at the end of most commands) shows the same treatment for the active Subject: once `model use` is run, `printContext()` fetches the Subject live and shows its full row — id, name, subject_type, has_perspective, has_self — the same fields `model list` shows, not just the bare id. This is global, not `model`-specific — any command ending in `printContext()` will show it. See `subjectContextLabel` in `cmd/root.go`.
