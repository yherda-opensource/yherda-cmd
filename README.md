...
Subject-generic commands (`cmd/model.go`) are the exception: they operate on a bare Subject id rather than a specific subtype, so they don't get their own `use`-and-cascade context slot unless a specific need is demonstrated — instead, every bare-Subject-id command (`model show`, `model dispositions list/create/delete`, `model states list/create/delete`, `model perspective get`, `model goals list/create`) takes an *optional* positional id and falls back to the active Subject (set by `model use`) when omitted, via `resolveSubjectID` in `cmd/model.go`. `model belief create --subject` is the one exception to the exception — it's an optional relationship flag on a different primary resource (the Belief being created), not a command whose primary target is a Subject id, so it doesn't participate in this fallback. `model states use` and `model goal use` are separate context slots in their own right, not bare-Subject-id commands: their active-context values are meaningfully reused across `states dispositions` and `steps` subcommands respectively. None of `model use`/`model states use`/`model goal use` clear Person/Place/Thing/State/Goal — a State, Goal, or generic Subject is scoped to whichever Subject already has it, not a replacement for the active Person/Place/Thing, so clearing them on `use` would lose context unnecessarily.

The active Subject is a breadcrumb stack (`Context.SubjectStack`), not a single slot — `model use <id>` pushes onto it, so drilling from a Person into one of its Dispositions (itself a Subject) into that Disposition's own Perspective and back out is a natural "list, use, crud, list, use, crud" loop without losing the parent. `model back` pops the stack, returning to whatever was active before the last `model use`; popping the last remaining entry is fine and leaves no active Subject (not an error — only popping an *already-empty* stack is). `model use` with no id prints the current breadcrumb trail rather than mutating anything. Passing an explicit id to any *other* bare-Subject-id command (`model perspective get 99`, `model dispositions list 99`, etc.) is a one-off override, not a navigation action — it resets the stack to just that id via `resolveSubjectID`, discarding whatever trail existed, rather than leaving old context silently underneath it.
