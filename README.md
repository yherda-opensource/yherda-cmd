...
| `cmd/ideas.go` | Idea listing + setting the active idea. Read-only: YOS-87 removed `ideas create`. |
...
| `cmd/identity.go` | Identity (character) listing. Read-only: YOS-87 removed `identity create`. |
| `cmd/person.go`, `cmd/place.go`, `cmd/setting.go`, `cmd/thing.go`, `cmd/disposition.go` | Person/place/thing entity listing and their settings and dispositions. Read-only: YOS-87 removed each resource's `create` command. |
| `cmd/model.go` | The `model` command family — Subject-generic commands (`show`, `list`, `use`, `back`, `dispositions list`, `states list`/`use`, `states dispositions list`) that operate on the platform's Subject base class by bare id, working the same way regardless of concrete subtype. Distinct from the per-type resource files above. Read-only: YOS-87 removed `dispositions create/delete`, `states create/delete`, and `states dispositions set/unset` — the CLI's OAuth2 client no longer carries a scope any of those endpoints accept (GEN-627). |
| `cmd/model_perspective.go` | `model perspective contexts list` — list the Contexts attached to a Subject's Perspective (`ContextPerspective`). Read-only: YOS-87 removed `model perspective get` (a `get_or_create` — materializing a Perspective was really a write despite the name) and `contexts add/update/remove`. A genuinely read-only perspective-resolution replacement is a future, not-yet-designed piece of work. |
| `cmd/context.go` | `context list`/`use`, and `context belief list` — a context-first read of the same `BeliefContext` API. Read-only: YOS-87 removed `context create` and `context belief add/remove`. |
| `cmd/model_goal.go` | `model goals list`, `model goal use`, `model steps list` — read-only Goal/Step listing on a Subject, plus the active-goal context slot. YOS-87 removed `model goals create`, `model steps create`, and the shared `createGoalOnSubject` helper they used. |
...
- **Active context** — a `.yherda` file in the current working directory, holding the active workspace, idea, person, place, thing, and context. Most resource commands fall back to this context when an explicit ID flag isn't passed, and `useParent` (in `cmd/root.go`) persists `use`d resources back into it. Already directory-scoped regardless of `YHERDA_SANDBOX`. `ideas use` clears person/place/thing/context, since all four are scoped to a specific idea; `context use` itself doesn't cascade — Context is independent, not a descendant of person/place/thing.
...
**YOS-87 (2026-08-14): this CLI is read-only against the platform API today.** GEN-627 permanently caps the CLI's OAuth2 client to read-only Idea Owner scope plus Collaborator scope — and no currently-registered command targets a Collaborator-scoped endpoint (`/forminstance/`), so every write (`create`/`add`/`update`/`remove`/`delete`/`set`/`unset`, and the deceptively-named `model perspective get`, which was a `POST` under the hood) has been removed rather than left to silently 403. A future `yherda studio *` command family, built against `/forminstance/` and other Collaborator-scoped endpoints, is the planned way writes come back — not yet designed. The historical design rationale below (belief-attachment specificity, `model add identity`, confirmation prompts) describes commands that no longer exist, kept for context on *why* the CLI was shaped this way, not as a guide to current behavior.
...
2. Subcommands for `list`, `show <id>`, and `use <id>` (not every resource needs all three — pick what fits). `create` (and other write subcommands) are gone as of YOS-87 — see above.
3. Each `RunE` builds an authenticated client with `mustClient()`, falls back to the active context for parent IDs when no flag is passed, and calls `client.Get` against the relevant REST path.
...
Subject-generic commands (`cmd/model.go`) are the exception: they operate on a bare Subject id rather than a specific subtype, so they don't get their own `use`-and-cascade context slot unless a specific need is demonstrated — instead, every bare-Subject-id command (`model show`, `model dispositions list`, `model states list`, `model goals list`) takes an *optional* positional id and falls back to the active Subject (set by `model use`) when omitted, via `resolveSubjectID` in `cmd/model.go`. `model states use` and `model goal use` are separate context slots in their own right, not bare-Subject-id commands: their active-context values are meaningfully reused across `states dispositions` and `steps` subcommands respectively. None of `model use`/`model states use`/`model goal use` clear Person/Place/Thing/State/Goal — a State, Goal, or generic Subject is scoped to whichever Subject already has it, not a replacement for the active Person/Place/Thing, so clearing them on `use` would lose context unnecessarily.

The active Subject is a breadcrumb stack (`Context.SubjectStack`), not a single slot — `model use <id>` pushes onto it, so drilling from a Person into one of its Dispositions (itself a Subject) and back out is a natural "list, use, list, use" loop without losing the parent. `model back` pops the stack, returning to whatever was active before the last `model use`; popping the last remaining entry is fine and leaves no active Subject (not an error — only popping an *already-empty* stack is). `model use` with no id prints the current breadcrumb trail rather than mutating anything. Passing an explicit id to any *other* bare-Subject-id command (`model dispositions list 99`, etc.) is a one-off override, not a navigation action — it resets the stack to just that id via `resolveSubjectID`, discarding whatever trail existed, rather than leaving old context silently underneath it.
...
Every `model` command that acts on a bare Subject id prints a two-line footer after its own output, via `printContextWithSubject` in `cmd/root.go`: line 1 is the Subject record just acted on (`Subject: #<id> "<name>" (<subject_type>)`), line 2 is the usual `context: ...` row, with the `subject` field now showing that same full row — id, name, subject_type, has_perspective, has_self, the same fields `model list` shows — rather than just the bare id. Both lines skipped in `--json`/`--no-context` mode. This is visibility, not validation: no `model` command hard-blocks any Subject id or subtype, per the epic's no-type-gating principle — the goal is that a wrong-context id (e.g. one copied from `model list`'s idea-fallback table) is immediately visible, not silently accepted.

The full-row treatment in line 2 applies globally, not just to `model` commands — any command ending in plain `printContext()` shows the active Subject's full row (via `subjectContextLabel`) once `model use` has been run, the same way `state`/`goal`/etc. already appear once set. Only Subject gets a name lookup today; the other context fields (idea, person, place, thing, state, goal) still show as bare ids.
